...
export function ContactForm({ locale }: { locale: Locale }) {
  ...
  return (
    <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); setSubmitted(true); }} noValidate>
      <div><label className="booking-label" htmlFor="contact-name">{t.name}</label><input id="contact-name" className="booking-field" autoComplete="name" value={name} onChange={(e)=>setName(e.target.value)} /></div>
      <div><label className="booking-label" htmlFor="contact-email">{t.email}</label><input id="contact-email" type="email" autoComplete="email" inputMode="email" className="booking-field" value={email} onChange={(e)=>setEmail(e.target.value)} /></div>
      <div><label className="booking-label" htmlFor="contact-phone">{t.phone}</label><input id="contact-phone" type="tel" autoComplete="tel" inputMode="tel" className="booking-field" value={phone} onChange={(e)=>setPhone(e.target.value)} /></div>
      <div>
